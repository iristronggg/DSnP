...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
       size_t m = n % R_SIZE;
       // TODO
       MemRecycleList<T> *tmplist = &(_recycleList[m]);
       //if found in first
       if(tmplist->getArrSize() == n)
           return tmplist;
       while(tmplist->getNextList() != 0){
           tmplist = tmplist->getNextList();
           if(tmplist->getArrSize() == n)
               return tmplist;
       }
       //if not found arrSize == n
        MemRecycleList<T> *newlist = new MemRecycleList<T>(n);
       tmplist->setNextList(newlist);
       return newlist;
   ...
   T* getMem(size_t t) {
      ...
      //    #endif // MEM_DEBUG
      // TODO take from memory block
        else{
            if(!_activeBlock->getMem(t, ret)){ //no enough space
                size_t rn = _activeBlock->getRemainSize();
                if(rn >= toSizeT(S)){  //at least one object
                    //cout << "yes" << rn << endl << toSizeT(S);
