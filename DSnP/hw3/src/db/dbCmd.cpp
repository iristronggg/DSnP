   ...
   // check option
    ...
    if(opts.size() == 2){
        if(isValidVarName(opts[0]))
            key = opts[0];
        else return CmdExec::errorOption(CMD_OPT_ILLEGAL, opts[0]);
        if (!myStr2Int(opts[1], val))
            return CmdExec::errorOption(CMD_OPT_ILLEGAL, opts[1]);
        DBJsonElem jsel(key, val);
        if(!dbjson.add(jsel)){
            cerr << "Error: Element with key \"" << key << "\" already exists!!" << endl;
            return CMD_EXEC_ERROR;
        }
        else dbjson.add(jsel);
    }
...
{  
   // TODO...
    ...
    string token;
    if (!CmdExec::lexSingleOption(option, token, true)) //token is optional
        return CMD_EXEC_ERROR;
    if(token.size() == 0){
        cout << dbjson;
        ...
        size_t index;
        if(dbjson.find_key_idx(token, index)){
