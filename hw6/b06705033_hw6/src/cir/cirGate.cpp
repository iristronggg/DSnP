...
CirGate::reportFanout(int level) const
{
    assert (level >= 0);
    setGlobalRef();
    printFanout(this, false, level, 0);
}
...
void CirGate::printFanout(const CirGate* node, bool inv, int level, int now_level) const
{
    assert (level >= 0);
    bool oinv = false;
    ...
    if(isGlobalRef()){
        ...
        for(unsigned i = 0; i < node->_outList.size(); i++){
            oinv = false;
            if((node->getID() == node->_outList[i]->_fanin0) && (node->_outList[i]->_invPhase0 == true)){
                oinv = true;
            }
            else if((node->getID() == node->_outList[i]->_fanin1) && (node->_outList[i]->_invPhase1 == true)){
                oinv = true;
            }
            
            node->_outList[i]->printFanout(node->_outList[i], oinv, level, now_level+1);
