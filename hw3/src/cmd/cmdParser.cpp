...
{
   // TODO...
    _dofile = new ifstream(dof.c_str()); //current dofile
    if(!(*_dofile)){                    //fail to open
        if(_dofileStack.size() != 0)
            _dofile = _dofileStack.top(); //back to recent dofile
        else _dofile = 0;
        return false;
        
    }
    _dofileStack.push(_dofile);  //current dofile read in
    return true;
}
...
CmdParser::parseCmd(string& option)
{
   ...
   // TODO...
    string first;
    string check; //check if empty
    size_t end = myStrGetTok(str, first);
    myStrGetTok(str,check,end);
    if(check != "")
        option = str.substr(end);
    if(getCmd(first) == 0){
        cerr << "Illegal command!! (" << first << ")" << endl;
        return NULL;
    }
